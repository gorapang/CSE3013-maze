...
bool ofApp::readFile()
{
	...
	if (openFileResult.bSuccess) {
		...
		if (pos != string::npos && pos != 0 && filePath.substr(pos + 1) == "maz") {
			...
			//오류 방지를 위해 -1로 초기화 (방이 아닌 부분에는 -1이 있어야 함, adj의 원소 중 -1이 있으면 이상한거)
			for (int i = 0; i < HEIGHT; i++)
			...
			//input의 각 방에 번호 부여 아스키 코드 127 초과하기 때문에 미로 커지면 dfs 동작 안함(알맞은 숫자가 안들어있어서)
			//char**가 아닌 int** rooms에 방 번호 저장하기
		...
		/*	printf("rooms\n");
			...
			//	for (int j = 1; j < WIDTH; j+=2)
			//	{
			//		if (input[i][j + 1] == ' ') //우측 벽 없음
			//		{
			//			//input[i][j]와 input[i][j+2] 연결
			//			room1 = input[i][j] - '0';
			//			room2 = input[i][j + 2] - '0';
			//			adjacency[room1][room2] = 1;
			//			adjacency[room2][room1] = 1; //대칭 처리
			//		}
			//		if (input[i + 1][j] == ' ') //아래 벽 없음
			//		{
			//			room1 = input[i][j] - '0';
			//			room2 = input[i + 2][j] - '0';
			//			adjacency[room1][room2] = 1;
			...
			for (int i = 1; i < HEIGHT; i += 2)
			{
				for (int j = 1; j < WIDTH; j += 2)
				{
					...
					{
						room1 = rooms[i][j]; //room1 = input[i][j] - '0'; 이렇게 하면 안됨(아스키 코드는 127까지밖에 없으니까, 그 이후는 이상한 값이 저장됨)
						room2 = rooms[i][j + 2];
			...
			read_flag = 1;
			if (isDFS == 1) isDFS = 0; //한 미로에서 dfs 후 다른 미로 open하면 바로 dfs되는것 방지
			if (isBFS == 1) isBFS = 0;
		}
...
bool ofApp::DFS(int v)//DFS탐색을 하는 함수
{
	//TO DO
	//DFS탐색을 하는 함수 (3주차)
	//isDFS = 1; //while문 안에서 return되니까 맨뒤에쓰면 안됨
	if (isBFS == 1) isBFS = 0;
	...
	while (!s.empty())
	{
		...
		//for (int i = 0; i < roomnum; i++) //우선순위 아래, 오른쪽, 왼쪽, 위
		//{
